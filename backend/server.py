...
@api_router.post("/transfers/{transfer_id}/receive-with-id")
async def receive_transfer_with_id(
    transfer_id: str,
    pin: str = Form(...),
    id_image: UploadFile = File(...),
    ...
    """Receive/complete transfer with ID image verification"""
    import json
    verification_data = json.loads(name_verification)
    
    # Check name match status
    if verification_data.get('match_status') == 'no_match':
        raise HTTPException(status_code=400, detail="لا يمكن تسليم الحوالة - الاسم غير مطابق")
    ...
    try:
        ...
        # Continue even if upload fails - we already verified the name
    ...
    await db.transfers.update_one(
        ...
        {'$set': {
            ...
            'name_verification': verification_data
        }}
    )
    ...
    return {
        ...
        'name_match_status': verification_data.get('match_status')
    }
        ...
        if not receiver_account_code:
            ...
            if incoming_commission > 0:
                ...
                journal_entry_commission = {
                    ...
                    'reference_type': 'commission_received',
                    'reference_id': transfer_id,
                    'created_by': receiving_agent_id,
                    'created_at': datetime.now(timezone.utc).isoformat(),
                    'is_cancelled': False
                }
    ...
    try:
        ...
        for row in sheet.iter_rows():
            for cell in row:
                ...
                # إذا كانت الخلية تحتوي على حدود كاملة، نبقي الحدود والخلفية على عنصر النص نفسه
                if has_top_border and has_bottom_border and has_left_border and has_right_border:
                    # الحدود والخلفية ستُطبق مباشرة على عنصر النص (لا نفعل شيء هنا)
                    pass
                
                # إنشاء خطوط فردية للحدود الجزئية (ليس كل الجوانب)
                elif has_any_border:
                    if has_top_border:
                        line_element = {
                            'id': str(element_id),
